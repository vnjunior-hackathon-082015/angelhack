(function () {
    ...
    function commonShareService($http, $window) {
      ...
      function getCurrentIdea(){
        if (!_currentIdea) {
          var value = $window.sessionStorage.currentIdea;
          _currentIdea = value ? JSON.parse(value) : null;
        }
        return _currentIdea;
      }

      function setCurrentIdea(param){
        var str = param;
        if (param) {
            str = JSON.stringify(param);
        }
        _currentIdea = param;
        $window.sessionStorage.currentIdea = str;
      }
